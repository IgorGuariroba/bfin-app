...
**Agente (assistente)**:
Programa externo (client MCP — Claude, ChatGPT, Cursor) que opera o núcleo financeiro de um `User` `pro` em seu nome, autenticado por um `ApiKey`. Não é uma feature embutida no app: é um principal delegado. Faz leitura e escrita no domínio (escopo e regras em ADR-0004), com `Transaction Source = agent`.
_Avoid_: assistente (preferir "agente" para o MCP), IA, bot.
...
> **Dev:** "Quando o dia chega, a **Baixa automática** marca a `Transaction` `diario` como paga?"
> **Domínio:** "Não — ela **exclui** o `diario` do dia corrente. O diário é a projeção de gasto possível, não um gasto real; excluído, o usuário lança o gasto real como `saida`."

> **Dev:** "`economia` abate o saldo igual `saida`?"
> **Domínio:** "Não. `economia` é guardado — sai do fluxo corrente e **não** abate saldo nem custo de vida. É o tipo com efeito contábil próprio, não um subtipo de gasto."
