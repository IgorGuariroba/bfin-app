import { pgTable, varchar, timestamp, text, integer, uniqueIndex, foreignKey, doublePrecision, boolean, index, primaryKey } from "drizzle-orm/pg-core"
...
export const accountMember = pgTable("AccountMember", {
	...
	uniqueIndex("AccountMember_inviteToken_key").using("btree", table.inviteToken.asc().nullsLast().op("text_ops")),
	index("AccountMember_memberId_idx").using("btree", table.memberId.asc().nullsLast().op("text_ops")),
	index("AccountMember_ownerId_idx").using("btree", table.ownerId.asc().nullsLast().op("text_ops")),
	foreignKey({
...
export const postTopic = pgTable("PostTopic", {
	id: text().primaryKey().notNull(),
	slug: text().notNull(),
	name: text().notNull(),
}, (table) => [
	uniqueIndex("PostTopic_name_key").using("btree", table.name.asc().nullsLast().op("text_ops")),
...
export const tagToTransaction = pgTable("_TagToTransaction", {
	a: text("A").notNull(),
	b: text("B").notNull(),
}, (table) => [
	...
	primaryKey({ columns: [table.a, table.b], name: "_TagToTransaction_AB_pkey"}),
]);

export const postTopics = pgTable("_PostTopics", {
	a: text("A").notNull(),
	b: text("B").notNull(),
}, (table) => [
	index().using("btree", table.b.asc().nullsLast().op("text_ops")),
